...
import { Search, X, Loader2, FolderOpen } from 'lucide-vue-next'
...
const results = ref<ProductCategory[]>([])
...
async function fetchCategories(search: string = '') {
  loading.value = true
  try {
    const res = await api.get<{ data: ProductCategory[] }>('/products/categories/index', { search })
  ...
  <div ref="containerRef" class="relative">
    ...
    <div class="relative">
      ...
      <input
        v-model="query"
        type="text"
        placeholder="Cari atau pilih kategori produk..."
    ...
    <div v-if="open && (results.length || loading)" class="absolute z-40 mt-1 w-full rounded-lg border border-gray-200 bg-white shadow-lg">
      <div class="flex items-center justify-between border-b border-gray-100 px-3 py-2">
        <span class="text-xs text-gray-400">Pilih kategori produk</span>
      ...
      <div v-else class="max-h-80 overflow-y-auto">
        ...
        >
          <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-orange-50">
            <FolderOpen class="h-4 w-4 text-orange-600" />
          </div>
          <div class="min-w-0 flex-1">
            <p class="truncate text-sm font-medium text-gray-900">{{ category.name }}</p>
            <p v-if="category.parent_name" class="truncate text-xs text-gray-500">
