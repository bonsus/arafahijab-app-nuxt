<script setup lang="ts">
import {
  ArrowLeft, Plus, Loader2, MapPin,
} from 'lucide-vue-next'

definePageMeta({ middleware: 'auth' })

const api = useApi()
const toast = useToast()
const router = useRouter()
const route = useRoute()

// Determine type from query param
const contactType = computed(() => {
  const q = route.query.type as string
  return q === 'supplier' ? 'supplier' : 'customer'
})
const isCustomer = computed(() => contactType.value === 'customer')
const typeLabel = computed(() => isCustomer.value ? 'Pelanggan' : 'Supplier')
const backRoute = computed(() => isCustomer.value ? '/contact/customer' : '/contact/supplier')

const saving = ref(false)
const formErrors = ref<Record<string, string[]>>({})
const loadingRef = ref(true)

// Reference data
interface CategoryOption { id: string; name: string }
interface StoreOption { id: string; name: string }

const categories = ref<CategoryOption[]>([])
const stores = ref<StoreOption[]>([])

// Form
const form = reactive({
  store_id: '',
  customer_category_id: '',
  external_id: '',
  name: '',
  phone: '',
  email: '',
  username: '',
  password: '',
  type: contactType.value,
  status: 'active',
  login_status: 'active',
})

// Addresses — using postal_code to match useAddressSelect interface
interface AddressRow {
  name: string
  phone: string
  country: string
  province: string
  city: string
  district: string
  postal_code: string
  address: string
  primary: boolean
}

const addresses = ref<AddressRow[]>([])

function addAddress() {
  addresses.value.push({
    name: '',
    phone: '',
    country: '',
    province: '',
    city: '',
    district: '',
    postal_code: '',
    address: '',
    primary: addresses.value.length === 0,
  })
}

function removeAddress(index: number) {
  const wasPrimary = addresses.value[index]?.primary
  addresses.value.splice(index, 1)
  if (wasPrimary && addresses.value.length > 0 && addresses.value[0]) {
    addresses.value[0].primary = true
  }
}

function setPrimary(index: number) {
  addresses.value.forEach((a, i) => a.primary = i === index)
}

function getFieldError(field: string): string | undefined {
  return formErrors.value[field]?.[0]
}

async function fetchReferenceData() {
  loadingRef.value = true
  try {
    const [catRes, storeRes] = await Promise.all([
      api.get<{ data: CategoryOption[] }>('/customers/categories/index'),
      api.get<{ data: { data: StoreOption[] } }>('/stores/index', { per_page: '100' }),
    ])
    categories.value = catRes.data || []
    stores.value = storeRes.data?.data || storeRes.data || []

    // Auto-select "regular" category
    const regular = categories.value.find(c => c.name.toLowerCase() === 'regular')
    if (regular) {
      form.customer_category_id = regular.id
    }
  } catch {
    // silent
  } finally {
    loadingRef.value = false
  }
}

async function handleSubmit() {
  saving.value = true
  formErrors.value = {}
  try {
    const body: Record<string, any> = {
      store_id: form.store_id,
      customer_category_id: form.customer_category_id,
      external_id: form.external_id,
      name: form.name,
      phone: form.phone,
      email: form.email,
      type: contactType.value,
      status: form.status,
    }
    // Only include customer-specific fields
    if (isCustomer.value) {
      body.username = form.username
      body.password = form.password
      body.login_status = form.login_status
    }
    if (addresses.value.length) {
      // Map postal_code → zipcode for API
      body.addresses = addresses.value.map(a => ({
        name: a.name,
        phone: a.phone,
        country: a.country,
        province: a.province,
        city: a.city,
        district: a.district,
        zipcode: a.postal_code,
        address: a.address,
        primary: a.primary,
      }))
    }
    await api.post('/customers/create', body)
    toast.success(`${typeLabel.value} berhasil dibuat`)
    router.push(backRoute.value)
  } catch (err: any) {
    if (err.errors) formErrors.value = err.errors
    toast.error(err.message || `Gagal membuat ${typeLabel.value.toLowerCase()}`)
  } finally {
    saving.value = false
  }
}

onMounted(() => fetchReferenceData())
</script>

<template>
  <div class="space-y-6">
    <!-- Header -->
    <div class="flex items-center gap-3">
      <NuxtLink
        :to="backRoute"
        class="rounded-lg p-2 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600"
      >
        <ArrowLeft class="h-5 w-5" />
      </NuxtLink>
      <div>
        <h1 class="text-2xl font-bold text-gray-900">Tambah {{ typeLabel }}</h1>
        <p class="mt-0.5 text-sm text-gray-500">Buat data {{ typeLabel.toLowerCase() }} baru</p>
      </div>
    </div>

    <form @submit.prevent="handleSubmit">
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <!-- LEFT (2/3) -->
        <div class="space-y-6 lg:col-span-2">
          <!-- Basic Info -->
          <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-200 sm:p-6">
            <h2 class="mb-4 text-base font-semibold text-gray-900">Informasi Dasar</h2>
            <div class="space-y-4">
              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700">Nama <span class="text-red-500">*</span></label>
                <input v-model="form.name" type="text" class="form-input" placeholder="Nama customer" />
                <p v-if="getFieldError('name')" class="mt-1 text-xs text-red-500">{{ getFieldError('name') }}</p>
              </div>

              <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <label class="mb-1.5 block text-sm font-medium text-gray-700">Email</label>
                  <input v-model="form.email" type="email" class="form-input" placeholder="email@example.com" />
                  <p v-if="getFieldError('email')" class="mt-1 text-xs text-red-500">{{ getFieldError('email') }}</p>
                </div>
                <div>
                  <label class="mb-1.5 block text-sm font-medium text-gray-700">Telepon</label>
                  <input v-model="form.phone" type="text" class="form-input" placeholder="08xxxx" />
                  <p v-if="getFieldError('phone')" class="mt-1 text-xs text-red-500">{{ getFieldError('phone') }}</p>
                </div>
              </div>

              <div v-if="isCustomer" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <label class="mb-1.5 block text-sm font-medium text-gray-700">Username</label>
                  <input v-model="form.username" type="text" class="form-input" placeholder="Username" />
                  <p v-if="getFieldError('username')" class="mt-1 text-xs text-red-500">{{ getFieldError('username') }}</p>
                </div>
                <div>
                  <label class="mb-1.5 block text-sm font-medium text-gray-700">Password</label>
                  <input v-model="form.password" type="password" class="form-input" placeholder="Min. 8 karakter" />
                  <p v-if="getFieldError('password')" class="mt-1 text-xs text-red-500">{{ getFieldError('password') }}</p>
                </div>
              </div>

              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700">External ID</label>
                <input v-model="form.external_id" type="text" class="form-input" placeholder="ID dari sistem lain (opsional)" />
              </div>
            </div>
          </div>

          <!-- Addresses -->
          <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-200 sm:p-6">
            <div class="mb-4 flex items-center justify-between">
              <h2 class="text-base font-semibold text-gray-900">Alamat</h2>
              <button
                type="button"
                class="flex items-center gap-1.5 rounded-lg bg-primary-50 px-3 py-1.5 text-xs font-medium text-primary-700 transition-colors hover:bg-primary-100"
                @click="addAddress"
              >
                <Plus class="h-3.5 w-3.5" />
                Tambah Alamat
              </button>
            </div>

            <div v-if="!addresses.length" class="rounded-lg border border-dashed border-gray-300 p-6 text-center text-sm text-gray-400">
              <MapPin class="mx-auto mb-2 h-6 w-6" />
              Belum ada alamat. Klik tombol di atas untuk menambahkan.
            </div>

            <div v-else class="space-y-4">
              <AddressFormRow
                v-for="(addr, idx) in addresses"
                :key="idx"
                v-model="addresses[idx]!"
                :index="idx"
                @remove="removeAddress(idx)"
                @set-primary="setPrimary(idx)"
              />
            </div>
          </div>
        </div>

        <!-- RIGHT (1/3) -->
        <div class="space-y-6">
          <!-- Type & Status -->
          <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
            <h2 class="mb-4 text-base font-semibold text-gray-900">Pengaturan</h2>
            <div class="space-y-4">
              <!-- <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700">Tipe</label>
                <input :value="typeLabel" type="text" class="form-input bg-gray-50" disabled />
              </div> -->
              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700">Status</label>
                <select v-model="form.status" class="form-input">
                  <option value="active">Aktif</option>
                  <option value="inactive">Nonaktif</option>
                </select>
              </div>
              <div v-if="isCustomer">
                <label class="mb-1.5 block text-sm font-medium text-gray-700">Status Login</label>
                <select v-model="form.login_status" class="form-input">
                  <option value="active">Aktif</option>
                  <option value="inactive">Nonaktif</option>
                </select>
              </div>
            </div>
          </div>

          <!-- Store -->
          <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
            <h2 class="mb-4 text-base font-semibold text-gray-900">Toko</h2>
            <div class="space-y-4">
              <div>
                <!-- <label class="mb-1.5 block text-sm font-medium text-gray-700">Toko</label> -->
                <select v-model="form.store_id" class="form-input">
                  <option value="">Pilih toko</option>
                  <option v-for="s in stores" :key="s.id" :value="s.id">{{ s.name }}</option>
                </select>
                <p v-if="getFieldError('store_id')" class="mt-1 text-xs text-red-500">{{ getFieldError('store_id') }}</p>
              </div>
            </div>
          </div>
        </div>
      </div>

      <!-- Sticky bottom -->
      <div class="sticky bottom-0 -mx-4 mt-6 border-t border-gray-200 bg-white/95 px-4 py-4 backdrop-blur-sm sm:-mx-6 sm:px-6">
        <div class="flex items-center justify-end gap-3">
          <NuxtLink
            :to="backRoute"
            class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50"
          >
            Batal
          </NuxtLink>
          <button
            type="submit"
            :disabled="saving"
            class="flex items-center gap-2 rounded-lg bg-primary-600 px-5 py-2 text-sm font-semibold text-white transition-colors hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
            <Loader2 v-if="saving" class="h-4 w-4 animate-spin" />
            {{ saving ? 'Menyimpan...' : 'Simpan' }}
          </button>
        </div>
      </div>
    </form>
  </div>
</template>

<style scoped>
@reference "~/assets/css/main.css";

.form-input {
  @apply w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 transition-colors focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20;
}
</style>
