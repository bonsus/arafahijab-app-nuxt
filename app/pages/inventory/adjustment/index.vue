...
import {
  ...
  Pencil, RefreshCw, ClipboardList, TrendingUp, TrendingDown,
  PackageCheck, Layers, Download, Loader2, ChevronDown,
...
async function fetchItems() {
  ...
  try {
    ...
    const res = await api.get<{ data: Paginated }>('/inventories/stock-adjustments/index', params)
...
async function handleDelete(item: StockAdjustment) {
  ...
  try {
    await api.delete(`/inventories/stock-adjustments/${item.id}`)
...
async function exportData(endpoint: string) {
  ...
  try {
    ...
    const name = (endpoint.split('/').pop() || 'export').replace(/-/g, '_')
...
<template>
  <div class="space-y-5">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-xl font-bold text-gray-900 sm:text-2xl">Penyesuaian Stok</h1>
        <p class="mt-0.5 text-sm text-gray-500">Kelola koreksi stok di gudang</p>
      </div>
      <NuxtLink
        to="/inventory/adjustment/create"
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-8">
      <!-- Total -->
      <div class="col-span-2 sm:col-span-2 rounded-xl bg-white px-4 py-3.5 shadow-xs ring-1 ring-gray-200">
        <p class="text-xs font-medium text-gray-500">Total Penyesuaian</p>
      ...
      <div class="col-span-2 sm:col-span-2 rounded-xl bg-white px-4 py-3.5 shadow-xs ring-1 ring-gray-200">
        ...
        <p class="mt-1.5 text-xs text-gray-400">Nilai koreksi stok</p>
      </div>
      <!-- Stok Masuk -->
      ...
      <div class="rounded-xl bg-white px-4 py-3.5 shadow-xs ring-1 ring-gray-200">
        ...
        <p class="mt-1.5 text-xs text-gray-400">Unit keluar</p>
      </div>
      <!-- Produk -->
      <div class="rounded-xl bg-white px-4 py-3.5 shadow-xs ring-1 ring-gray-200">
        <div class="flex items-center justify-between">
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <div class="relative" @click.stop>
        ...
        >
          <button class="flex w-full items-center gap-2 px-4 py-2.5 text-sm text-gray-700 transition-colors hover:bg-gray-50" @click="exportData('/inventories/stock-adjustments/export')">
            Penyesuaian Stok
          </button>
          <button class="flex w-full items-center gap-2 px-4 py-2.5 text-sm text-gray-700 transition-colors hover:bg-gray-50" @click="exportData('/inventories/stock-adjustments/export-with-items')">
            Penyesuaian Stok With Items
          </button>
        </div>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full min-w-[700px] text-sm">
          ...
          <tbody v-else-if="!items.length">
            <tr>
              <td colspan="7" class="px-4 py-16 text-center">
                ...
                <p class="text-sm text-gray-500">Belum ada penyesuaian stok</p>
                <NuxtLink
                  to="/inventory/adjustment/create"
          ...
          <tbody v-else>
            ...
            >
              <td class="px-4 py-3">
                <NuxtLink :to="`/inventory/adjustment/${item.id}`" class="font-semibold text-primary-600 hover:underline">
                  {{ item.no }}
                </NuxtLink> 
              </td>
              ...
              <td class="px-4 py-3">
                <div class="flex items-center justify-end gap-1">
                  ...
                  <NuxtLink
                    ...
                    :to="`/inventory/adjustment/create?edit=${item.id}`"
