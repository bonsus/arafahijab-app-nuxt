...
function toLocalDatetime(rfc3339: string): string {
  if (!rfc3339) return ''
...
async function loadPromotion() {
  ...
  try {
    ...
    form.start_date = toLocalDatetime(d.start_date)
    form.end_date = toLocalDatetime(d.end_date)
    form.status = d.status || 'draft'
    form.coupon_code = d.coupon_code || ''
    form.discount_type = d.discount_type || 'percentage'
    form.discount_value = d.discount_value || null
    form.max_discount_value = d.max_discount_value || null
    form.item_type = d.item_type || 'all'
    form.item_operator = d.item_operator || 'and'
    form.customer_type = d.customer_type || 'all'
    form.terms = d.terms || ''
    form.min_spend = d.min_spend || null
    form.min_qty = d.min_qty || null
    form.multiple_use = d.multiple_use || false
    form.max_use = d.max_use || null
    form.max_use_per_customer = d.max_use_per_customer || null
    form.internal_visibility = d.internal_visibility || false
    form.website_visibility = d.website_visibility || false
    form.app_visibility = d.app_visibility || false

    // Populate items
    if (d.items?.length) {
      items.value = d.items.map((it: any) => ({
        item_role: it.item_role || 'condition',
        product_id: it.product_id || '',
        sku_id: it.sku_id || '',
        product_category_id: it.product_category_id || '',
        discount_value: it.discount_value || null,
        max_discount_value: it.max_discount_value || null,
        min_qty: it.min_qty || null,
        max_qty: it.max_qty || null,
        qty: it.qty || null,
        product_name: it.product?.name || it.product_name || '',
        sku: it.sku?.sku || it.sku || '',
        category_name: it.product_category?.name || it.category_name || '',
      }))
    }

    // Populate customers
    if (d.customers?.length) {
      customerCategories.value = d.customers
        .filter((c: any) => c.customer_category_id && !c.customer_id)
        .map((c: any) => ({ 
          customer_category_id: c.customer_category_id,
          name: c.customer_category?.name || c.name || '',
        }))
      customerIndividuals.value = d.customers
    ...
    if (d.couriers?.length) {
      couriers.value = d.couriers.map((c: any) => ({
...
const backRoute = computed(() => backRoutes[form.type] || '/promotion/discount')
...
async function handleSubmit() {
  ...
  try {
    ...
    if (form.customer_type === 'specific') {
      ...
      body.customers = merged
    }
    else body.customers = []
    if (form.type === 'shipping') {
      body.couriers = couriers.value
      body.areas = areas.value
    }

    await api.put(`/sales/promotions/${promoId}`, body)
    ...
    toast.error(err.message || 'Gagal memperbarui promosi')
  }
  finally {
    saving.value = false
  }
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-2xl font-bold text-gray-900">Edit {{ typeLabels[form.type] || 'Promosi' }}</h1>
        <p class="mt-0.5 text-sm text-gray-500">Perbarui data promosi</p>
      </div>
    </div>
    ...
    <form v-else @submit.prevent="handleSubmit">
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-6 lg:col-span-2">
          ...
          <div v-if="form.type !== 'product_free'" class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-200 sm:p-6">
          ...
          <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-200 sm:p-6">
            ...
            <div class="space-y-4">
              <div v-if="form.type !== 'shipping'" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                ...
                <div v-if="form.type === 'product_free'">
          ...
          >
            ...
            <div class="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                <label class="mb-1.5 block text-xs font-medium text-gray-700">
                  {{ form.type === 'product_free' ? 'Produk/SKU Syarat' : 'Pilih Produk/SKU' }}
                </label>
                <AppProductSkuPicker 
              ...
              <div>
                <label class="mb-1.5 block text-xs font-medium text-gray-700">
                  {{ form.type === 'product_free' ? 'Kategori Syarat' : 'Pilih Kategori' }}
                </label>
            ...
            <div v-if="form.type === 'product_free'" class="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                ...
                <div class="flex items-start gap-3">
                  <div class="min-w-0 flex-1">
                    ...
                    <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
                      <div v-if="form.type === 'discount' && item.item_role === 'condition'">
        ...
        <div class="space-y-6">
          ...
          <!-- Visibility Card -->
          <div v-if="form.type === 'checkout'" class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
          ...
          <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
            ...
            <dl class="space-y-2 text-sm">
              <div class="flex justify-between">
                ...
                <dd class="font-medium text-gray-900">{{ typeLabels[form.type] || form.type }}</dd>
              </div>
              <div v-if="form.discount_value && form.type !== 'product_free'" class="flex justify-between">
              ...
              <div class="flex justify-between">
                ...
                <dd class="font-medium text-gray-900">{{ items.length }} item</dd>
              </div>
