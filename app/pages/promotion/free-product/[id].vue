...
import {
  ArrowLeft, Calendar, Edit2, Loader2, Gift, Power, Tag, Trash2,
  Clock, Ticket, Users, Box, ShoppingBag,
  Globe, Smartphone, Building2, Package,
...
async function loadPromotion() {
  loading.value = true
  try {
    const res = await api.get(`/promotions/product-frees/${promotionId.value}`) as any
...
async function loadOrders() {
  ...
  try {
    const res = await api.get(`/promotions/product-frees/${promotionId.value}/orders`) as any
...
async function toggleStatus(status: string) {
  ...
  toggling.value = true
  try {
    await api.put(`/promotions/product-frees/${promotionId.value}/update-status`, { status })
    if (promotion.value) {
...
async function handleDelete() {
  ...
  deleting.value = true
  try {
    await api.delete(`/promotions/product-frees/${promotionId.value}`)
...
function statusLabel(status: string): string {
  ...
  if (status === 'active') {
    if (new Date(promotion.value.date_end) < new Date()) return 'Berakhir'
    if (new Date(promotion.value.date_start) > new Date()) return 'Akan Datang'
    return 'Berlangsung'
  }
  return status || '-'
}
...
const groupedFreeSkus = computed(() => {
  if (!promotion.value?.product_frees) return {}
  const grouped: Record<string, FreeSku[]> = {}
  for (const sku of promotion.value.product_frees) {
    const pid = sku.product?.id || sku.product_id
    if (!grouped[pid]) grouped[pid] = []
    grouped[pid]!.push(sku)
  }
  return grouped
})
...
watch(activeTab, (newTab) => {
  if (newTab === 'orders' && orders.value.length === 0) loadOrders()
})
  ...
  <div class="space-y-6">
    ...
    <div class="flex items-start justify-between gap-4">
      <div class="flex items-center gap-3">
        <NuxtLink to="/promotion/free-product"
        ...
        <div v-if="!loading && promotion">
          ...
          <p class="mt-0.5 text-sm text-gray-500">Detail Promosi Produk Gratis</p>
        </div>
      ...
      <div v-if="!loading && promotion" class="flex items-center gap-2">
        <NuxtLink
          :to="`/promotion/free-product/create?edit=${promotionId}`"
    ...
    <div v-else-if="promotion" class="space-y-6">
      ...
      <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <p class="text-sm font-semibold text-gray-900">{{ formatDateTime(promotion.date_start) }}</p>
            <p class="text-xs text-gray-500">s/d {{ formatDateTime(promotion.date_end) }}</p>
          </div>
          ...
          <div>
            ...
            <p class="text-sm font-semibold text-gray-900">
              {{ promotion.used || 0 }} / {{ promotion.max_use && promotion.max_use > 0 ? promotion.max_use : '∞' }}
            </p>
            <p class="text-xs text-gray-500">
              Maks per customer: {{ promotion.max_use_per_customer && promotion.max_use_per_customer > 0 ? promotion.max_use_per_customer : '∞' }}
            </p>
          </div>
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-1.5">
              <span v-if="promotion.internal_visibility === 'active'" class="rounded bg-gray-100 px-2 py-0.5 text-[10px] font-bold text-gray-700">INT</span>
              <span v-if="promotion.web_visibility === 'active'" class="rounded bg-blue-100 px-2 py-0.5 text-[10px] font-bold text-blue-700">WEB</span>
              <span v-if="promotion.app_visibility === 'active'" class="rounded bg-green-100 px-2 py-0.5 text-[10px] font-bold text-green-700">APP</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- Tabs -->
      <div class="overflow-x-auto overflow-y-hidden border-b border-gray-200">
        <nav class="-mb-px flex space-x-6">
          <button
            v-for="tab in [{ key: 'overview', label: 'Ringkasan' }, { key: 'syarat', label: 'Syarat' }, { key: 'orders', label: 'Order' }, { key: 'logs', label: 'Log' }]"
            :key="tab.key"
      ...
      <div v-if="activeTab === 'overview'" class="space-y-6">
        <!-- Konfigurasi -->
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
          <h3 class="mb-4 text-lg font-semibold text-gray-900">Konfigurasi</h3>
          <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <dt class="text-sm font-medium text-gray-500">Multiple Reward</dt>
              <dd class="mt-1">
                <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold"
                  :class="promotion.is_multiple === 'active' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-600'"
                >
                  {{ promotion.is_multiple === 'active' ? 'Multiple' : 'Sekali' }}
                </span>
              </dd>
            </div>
            <div v-if="promotion.note">
        ...
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
          ...
          <div v-if="promotion.product_frees && promotion.product_frees.length > 0" class="space-y-3">
            <div
              v-for="[productId, skus] in Object.entries(groupedFreeSkus)"
              :key="productId"
              class="rounded-lg border border-green-200 bg-green-50/40 p-3"
            >
              <h4 class="mb-2 text-sm font-semibold text-gray-900">{{ skus[0]?.product?.name }}</h4>
              <table class="w-full text-left text-xs">
                <thead>
                  <tr class="border-b border-green-200">
                    <th class="px-2 py-1 text-gray-500">Thumbnail</th>
                    <th class="px-2 py-1 text-gray-500">SKU</th>
                    <th class="px-2 py-1 text-gray-500">Varian</th>
                    <th class="px-2 py-1 text-center text-gray-500">Qty Gratis</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="sku in skus" :key="sku.id" class="border-b border-green-100">
                    <td class="px-2 py-1">
                      <div class="h-7 w-7 overflow-hidden rounded-md bg-white ring-1 ring-gray-200">
                        <img v-if="sku.product?.thumbnail || sku.thumbnail" :src="sku.product?.thumbnail || sku.thumbnail" class="h-full w-full object-cover" />
                        <Box v-else class="h-full w-full p-1 text-gray-400" />
                      </div>
                    </td>
                    <td class="px-2 py-1 font-mono text-gray-700">{{ sku.sku }}</td>
                    <td class="px-2 py-1 text-gray-500">{{ formatVariants(sku.variants) }}</td>
                    <td class="px-2 py-1 text-center font-semibold text-green-700">{{ sku.qty }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>
          <p v-else class="text-sm text-gray-500">Tidak ada produk gratis</p>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'syarat'" class="space-y-6">
        ...
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
          ...
          <p v-else class="text-sm text-gray-500">Berlaku untuk semua kategori customer</p>
        </div>
        ...
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
          <h3 class="mb-4 flex items-center gap-2 text-lg font-semibold text-gray-900">
            ...
            Konfigurasi Produk Syarat
          </h3>
          <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <dt class="text-sm font-medium text-gray-500">Tipe Item</dt>
            ...
            <div v-if="promotion.item_type !== 'all'">
              ...
              <dd class="mt-1">
                <span class="inline-flex items-center gap-1 rounded-md bg-gray-100 px-2 py-1 text-xs font-medium text-gray-700">
                  ...
                  <span class="text-gray-500">{{ promotion.item_operator === 'or' ? '(Salah satu)' : '(Semua)' }}</span>
                </span>
              </dd>
            </div>
          </dl>
        ...
        <div v-if="promotion.item_type !== 'all'" class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
          ...
          <!-- Categories -->
          <div v-if="promotion.item_type === 'category' && promotion.product_categories" class="space-y-2">
            <div
            ...
            <p v-if="!promotion.product_categories.length" class="py-4 text-center text-sm text-gray-500">Tidak ada kategori</p>
          </div>
          ...
          <div v-else-if="promotion.item_type === 'product' && promotion.products" class="space-y-2">
            ...
            >
              ...
              <span class="ml-auto text-xs text-gray-500">
                {{ product.qty ? `Min Qty: ${product.qty}` : '' }}
              </span>
            </div>
            <p v-if="!promotion.products.length" class="py-4 text-center text-sm text-gray-500">Tidak ada produk</p>
          </div>

          <!-- SKUs -->
          <div v-else-if="promotion.item_type === 'sku' && promotion.products" class="space-y-3">
            <div
              v-for="product in promotion.products"
              :key="product.id"
              class="rounded-lg border border-gray-200 p-3"
            >
              <h4 class="mb-2 text-sm font-semibold text-gray-900">{{ product.name }}</h4>
              <table class="w-full text-left text-xs">
                <thead>
                  <tr class="border-b border-gray-200">
                    <th class="px-2 py-1 text-gray-500">SKU</th>
                    <th class="px-2 py-1 text-gray-500">Varian</th>
                    <th class="px-2 py-1 text-center text-gray-500">Min Qty</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="sku in product.skus" :key="sku.id" class="border-b border-gray-100">
                    <td class="px-2 py-1">
                      <div class="flex items-center gap-2">
                        <div class="h-6 w-6 overflow-hidden rounded ring-1 ring-gray-200">
                          <img v-if="sku.thumbnail" :src="sku.thumbnail" class="h-full w-full object-cover" />
                          <Box v-else class="h-full w-full p-1 text-gray-400" />
                        </div>
                        <span class="font-mono text-gray-800">{{ sku.sku }}</span>
                      </div>
                    </td>
                    <td class="px-2 py-1 text-gray-500">{{ formatVariants(sku.variants) }}</td>
                    <td class="px-2 py-1 text-center">{{ sku.qty || '-' }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <p v-if="!promotion.products.length" class="py-4 text-center text-sm text-gray-500">Tidak ada SKU</p>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'orders'">
        <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
          <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-900">Order yang Menggunakan Promosi Ini</h3>
            <p class="mt-1 text-sm text-gray-500">Daftar transaksi dengan produk gratis dari promosi ini</p>
          </div>
            ...
            >
              <div class="flex items-center gap-4">
                <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-green-50">
                  <ShoppingBag class="h-6 w-6 text-green-600" />
                </div>
                <div>
              ...
              <div class="text-right">
                <p class="text-sm font-semibold text-gray-900">Rp {{ formatPrice(order.total) }}</p>
              </div>
            </div>
          </div>
      ...
      <div v-if="activeTab === 'logs'">
        <div v-if="promotion.logs && promotion.logs.length > 0" class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
          ...
          <div class="divide-y divide-gray-100">
            ...
            >
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="mt-0.5 text-[10px] text-gray-400">{{ formatDateTime(log.created_at) }}</p>
              </div>
            </div>
          </div>
        </div>
        <div v-else class="rounded-xl bg-white p-6 text-center shadow-sm ring-1 ring-gray-200">
          <Clock class="mx-auto h-8 w-8 text-gray-300" />
