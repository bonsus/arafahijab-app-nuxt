...
import {
  ...
  Package, Loader2, Check, X, Pencil, RefreshCw, Download, ChevronDown,
...
async function exportData(endpoint: string) {
  ...
  try {
    ...
    const name = (endpoint.split('/').pop() || 'purchase_receipt').replace(/-/g, '_')
          ...
          <div class="relative" @click.stop>
            ...
            >
              <button
                ...
                @click="exportData('/sales/purchase-order-export/purchase-receipt')"
              >
                Purchase Receipt
              </button>
              <button
                class="flex w-full items-center gap-2 px-4 py-2.5 text-sm text-gray-700 transition-colors hover:bg-gray-50"
                @click="exportData('/sales/purchase-order-export/purchase-receipt-with-items')"
              >
