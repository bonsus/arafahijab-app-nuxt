...
interface CourierItem { courier_code: string; courier_name: string; items: TrendItem[] }
interface MetricDef { key: string; label: string }
interface AnalysisData {
  title: string; default_metric: string
  available_metrics: MetricDef[]
  group_by: string; items: CourierItem[]; total_per_group: TrendItem[]
}
...
async function fetchData() {
  loading.value = true
  try {
    const res = await api.get<{ data: { analysis: AnalysisData } }>('/reports/sales/analysis-per-courier', buildParams())
    ...
    activeItems.value = (analysisData.value?.items || []).map(c => c.courier_code)
  }
  catch (err: any) { toast.error(err.message || 'Gagal memuat data analisis per kurir') }
...
async function exportData() {
  ...
  try {
    const endpoint = '/reports/sales/analysis-per-courier/export'
    ...
    link.download = `analysis_per_courier_${date}_${hhmm}.xlsx`
...
const chartSeries = computed(() => {
  ...
  const result: any[] = analysisData.value.items
    .filter(c => activeItems.value.includes(c.courier_code))
    .map((item) => {
      const origIdx = analysisData.value!.items.findIndex(c => c.courier_code === item.courier_code)
      return {
...
function toggleItem(code: string) {
  const idx = activeItems.value.indexOf(code)
  if (idx >= 0) { if (activeItems.value.length > 1) activeItems.value.splice(idx, 1) }
  else { activeItems.value.push(code) }
}
...
<template>
  <div class="space-y-5">
    ...
    <div class="flex items-start justify-between gap-4">
      <div>
        <h1 class="text-xl font-bold text-gray-900 sm:text-2xl">Analisis Per Kurir</h1>
        <p class="mt-0.5 text-sm text-gray-500">Bandingkan performa penjualan berdasarkan kurir pengiriman</p>
      </div>
    </div>
    ...
    <div class="rounded-xl bg-white p-5 shadow-xs ring-1 ring-gray-200">
      <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
        <p class="font-semibold text-gray-900">Tren per Kurir</p>
        <div class="flex flex-wrap items-center gap-2">
      ...
      <div v-if="analysisData?.items.length" class="mb-2 flex flex-wrap items-center gap-1.5">
        <button
          ...
          class="rounded-full border px-2.5 py-1 text-xs font-medium transition-all"
          :class="activeItems.includes(item.courier_code) ? 'border-transparent text-white shadow-sm' : 'border-gray-200 bg-white text-gray-500 hover:bg-gray-50'"
          :style="activeItems.includes(item.courier_code) ? { backgroundColor: ITEM_COLORS[i % ITEM_COLORS.length] } : {}"
          @click="toggleItem(item.courier_code)"
        >
          {{ item.courier_name }}
        </button>
        <button
    ...
    <div class="rounded-xl bg-white shadow-xs ring-1 ring-gray-200">
      <div class="border-b border-gray-100 px-4 py-3">
        ...
        <p class="mt-0.5 text-xs text-gray-400">Tanggal sebagai baris · Kurir sebagai kolom · Tren vs periode sebelumnya</p>
      </div>
