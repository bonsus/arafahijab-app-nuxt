...
interface StoreItem { store_id: string; shop_name: string; items: TrendItem[] }
interface MetricDef { key: string; label: string }
interface AnalysisData {
  title: string; default_metric: string
  available_metrics: MetricDef[]
  group_by: string; items: StoreItem[]; total_per_group: TrendItem[]
}
...
async function fetchData() {
  loading.value = true
  try {
    const res = await api.get<{ data: { analysis: AnalysisData } }>('/reports/sales/analysis-per-store', buildParams())
    ...
    activeItems.value = (analysisData.value?.items || []).map(s => s.store_id)
  }
  catch (err: any) { toast.error(err.message || 'Gagal memuat data analisis per toko') }
...
async function exportData() {
  ...
  try {
    const endpoint = '/reports/sales/analysis-per-store/export'
    ...
    link.download = `analysis_per_store_${date}_${hhmm}.xlsx`
...
const chartSeries = computed(() => {
  ...
  const result: any[] = analysisData.value.items
    .filter(s => activeItems.value.includes(s.store_id))
    .map((item) => {
      const origIdx = analysisData.value!.items.findIndex(s => s.store_id === item.store_id)
      return {
        name: item.shop_name,
...
function toggleItem(storeId: string) {
  const idx = activeItems.value.indexOf(storeId)
  if (idx >= 0) { if (activeItems.value.length > 1) activeItems.value.splice(idx, 1) }
  else { activeItems.value.push(storeId) }
}
...
<template>
  <div class="space-y-5">
    ...
    <div class="flex items-start justify-between gap-4">
      <div>
        <h1 class="text-xl font-bold text-gray-900 sm:text-2xl">Analisis Per Toko</h1>
        <p class="mt-0.5 text-sm text-gray-500">Bandingkan performa penjualan berdasarkan toko</p>
      </div>
    </div>
    ...
    <div class="rounded-xl bg-white p-5 shadow-xs ring-1 ring-gray-200">
      <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
        <p class="font-semibold text-gray-900">Tren per Toko</p>
        <div class="flex flex-wrap items-center gap-2">
      ...
      <div v-if="analysisData?.items.length" class="mb-2 flex flex-wrap items-center gap-1.5">
        <button
          ...
          :key="item.store_id"
          class="rounded-full border px-2.5 py-1 text-xs font-medium transition-all"
          :class="activeItems.includes(item.store_id) ? 'border-transparent text-white shadow-sm' : 'border-gray-200 bg-white text-gray-500 hover:bg-gray-50'"
          :style="activeItems.includes(item.store_id) ? { backgroundColor: ITEM_COLORS[i % ITEM_COLORS.length] } : {}"
          @click="toggleItem(item.store_id)"
        >
          {{ item.shop_name }}
        </button>
        <button
    ...
    <div class="rounded-xl bg-white shadow-xs ring-1 ring-gray-200">
      <div class="border-b border-gray-100 px-4 py-3">
        ...
        <p class="mt-0.5 text-xs text-gray-400">Tanggal sebagai baris · Toko sebagai kolom · Tren vs periode sebelumnya</p>
      </div>
