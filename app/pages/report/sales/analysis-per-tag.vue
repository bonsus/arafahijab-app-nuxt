...
async function fetchData() {
  loading.value = true
  try {
    const res = await api.get<{ data: { analysis: AnalysisData } }>('/reports/sales/analysis-per-tag', buildParams())
    ...
    activeItems.value = (analysisData.value?.items || []).map(t => t.tag)
  }
...
async function exportData() {
  ...
  try {
    const endpoint = '/reports/sales/analysis-per-tag/export'
    ...
    link.download = `analysis_per_tag_${date}_${hhmm}.xlsx`
...
const chartSeries = computed(() => {
  ...
  const result: any[] = analysisData.value.items
    .filter(t => activeItems.value.includes(t.tag))
    .map((item) => {
      const origIdx = analysisData.value!.items.findIndex(t => t.tag === item.tag)
      return {
        name: item.tag,
...
function toggleItem(tag: string) {
  const idx = activeItems.value.indexOf(tag)
  if (idx >= 0) { if (activeItems.value.length > 1) activeItems.value.splice(idx, 1) }
  else { activeItems.value.push(tag) }
}
...
<template>
  <div class="space-y-5">
    ...
    <div class="flex items-start justify-between gap-4">
      <div>
        <h1 class="text-xl font-bold text-gray-900 sm:text-2xl">Analisis Per Tag</h1>
        <p class="mt-0.5 text-sm text-gray-500">Bandingkan performa penjualan berdasarkan tag order</p>
      </div>
    </div>
    ...
    <div class="rounded-xl bg-white p-5 shadow-xs ring-1 ring-gray-200">
      <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
        <p class="font-semibold text-gray-900">Tren per Tag</p>
        <div class="flex flex-wrap items-center gap-2">
      ...
      <div v-if="analysisData?.items.length" class="mb-2 flex flex-wrap items-center gap-1.5">
        <button
          ...
          :key="item.tag"
          class="rounded-full border px-2.5 py-1 text-xs font-medium transition-all"
          :class="activeItems.includes(item.tag) ? 'border-transparent text-white shadow-sm' : 'border-gray-200 bg-white text-gray-500 hover:bg-gray-50'"
          :style="activeItems.includes(item.tag) ? { backgroundColor: ITEM_COLORS[i % ITEM_COLORS.length] } : {}"
          @click="toggleItem(item.tag)"
        >
          {{ item.tag }}
        </button>
        <button
    ...
    <div class="rounded-xl bg-white shadow-xs ring-1 ring-gray-200">
      <div class="border-b border-gray-100 px-4 py-3">
        ...
        <p class="mt-0.5 text-xs text-gray-400">Tanggal sebagai baris · Tag sebagai kolom · Tren vs periode sebelumnya</p>
      </div>
