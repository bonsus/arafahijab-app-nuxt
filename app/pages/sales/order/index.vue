...
const trackingOrder = ref<SalesOrder | null>(null)

function openTracking(order: SalesOrder) {
...
const trackingEvents = computed<TrackingEvent[]>(() => trackingOrder.value?.shipment?.data?.events || [])
...
const availableMenuActions = computed(() => {
  ...
  if (order.status === 'processing' && (order.sub_status === 'open')) {
                ...
                >
                  ...
                  <p v-if="order.shipment.tracking_no" class="font-mono text-xs text-gray-500 whitespace-nowrap flex items-center gap-1">
                    ...
                    <button
                      ...
                      type="button"
                      class="rounded p-0.5 text-gray-400 transition-colors hover:bg-primary-50 hover:text-primary-600"
        ...
        >
          <div class="flex max-h-[85vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
            ...
            <div class="flex items-start justify-between border-b border-gray-100 px-5 py-3.5">
              <div class="flex items-center gap-2">
                <img
                  v-if="trackingOrder.shipment?.aggregator"
                  :src="`/images/brands/${trackingOrder.shipment.aggregator}.webp`"
                  alt=""
                ...
                <div>
                  ...
                  <p class="mt-0.5 text-xs text-gray-500">
                    {{ [trackingOrder.shipment?.courier_name, trackingOrder.shipment?.service_name].filter(Boolean).join(' – ') }}
                    <span v-if="trackingOrder.shipment?.tracking_no" class="font-mono"> · {{ trackingOrder.shipment.tracking_no }}</span>
                  </p>
                </div>
              </div>
              <button
