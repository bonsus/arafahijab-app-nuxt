...
import {
  ...
  Pencil, Download, Loader2, ChevronDown,
...
async function exportData(endpoint: string) {
  ...
  try {
    ...
    const name = ('inbound_export').replace(/-/g, '_')
          ...
          <div class="relative" @click.stop>
            ...
            >
              <button
                ...
                @click="exportData('/warehouses/inbounds/export')"
              >
                Inbound
              </button>
              <button
                class="flex w-full items-center gap-2 px-4 py-2.5 text-sm text-gray-700 transition-colors hover:bg-gray-50"
                @click="exportData('/warehouses/inbounds/export-with-items')"
              >
                Inbound With Items
              </button>
            </div>
